...
$web1_url_cn    = $_POST['web1_url_cn'];
$web1_url_en    = $_POST['web1_url_en'];
$web1_url_jp    = $_POST['web1_url_jp'];
/******************** 上传图片处理  ******************/
$img_url_cn ='';
$img_url_en ='';
$img_url_jp ='';
...
$img1_url_en ='';
$img1_url_jp ='';
if($_FILES['img1_url_cn']['tmp_name']!=''){
    $img1_url_cn = upload_img($_FILES['img1_url_cn']['tmp_name'],$_FILES['img1_url_cn']['name'],"images");
}
if($_FILES['img1_url_en']['tmp_name']!=''){
    $img1_url_en = upload_img($_FILES['img1_url_en']['tmp_name'],$_FILES['img1_url_en']['name'],"images");
}
if($_FILES['img1_url_jp']['tmp_name']!=''){
    $img1_url_jp = upload_img($_FILES['img1_url_jp']['tmp_name'],$_FILES['img1_url_jp']['name'],"images");
} 
switch ($action){	
//------------------添加一条信息--------------------------
	case 'add':  
...
//------------------编辑一条信息--------------------------
	case 'edit':
		...
		$sql = "SELECT * FROM `".$db_pre."table_flash` WHERE id='".$id."' and typeof =1 ";
...
//------------------更新一条信息--------------------------
	case 'update': 
		...
		$sql = "UPDATE `".$db_pre."table_flash` SET  
					...
					.(empty($img1_url_cn)?"":"`img1_url_cn`='".$img1_url_cn."',")
					.(empty($img1_url_en)?"":"`img1_url_en`='".$img1_url_en."',")
					.(empty($img1_url_jp)?"":"`img1_url_jp`='".$img1_url_jp."',")
					."`web1_url_cn`='".$web1_url_cn."', 
					`web1_url_en`='".$web1_url_en."', 
					`web1_url_jp`='".$web1_url_jp."'
				WHERE id='".$id."'"; 
		if ($db->query($sql)){
...
//------------------审核--------------------------
	...
	default:
		...
		$sql = "SELECT * FROM `".$db_pre."table_flash` n WHERE 1 and class_id=1 and typeof =1 ";
		if ($keyword != ''){
...
$smarty->display("admin/web_table_top.html");
$runtime->stop_write();
?>